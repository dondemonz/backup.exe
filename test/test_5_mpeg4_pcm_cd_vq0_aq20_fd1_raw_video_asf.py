...
def test_30_H264():
    ...
    app = Application(backend="uia").start(r'"'+path_to_backupexe+'" --out "'+output_folder+out_file_name+'" --cam "' + cam_id + '" --from "'+begin_time+'" --to "' + end_time + '" --archive-path "'+path_to_archive+'" --video-codec MPEG4 --audio-codec PCM --span cd --video-quality 0 --audio-quality 20 --fps-divider 1 --raw-video').connect(title=title)
    ...
    assert hasher.hexdigest() == hash5_H264
...
def test_31_H263():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash5_H263
    os.remove(''+output_folder + out_file_name+'')
...
def test_32_MPEG4():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash5_MPEG4
    os.remove(''+output_folder + out_file_name+'')
...
def test_33_MJPEG():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash5_MJPEG
    os.remove(''+output_folder + out_file_name+'')
...
def test_61_MxPEG():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash5_MxPEG
    os.remove(''+output_folder + out_file_name+'')
