...
def test_30_H264():
    ...
    app = Application(backend="uia").start(r'"'+path_to_backupexe+'" --out "'+output_folder+out_file_name+'" --cam "' + cam_id + '" --from "'+begin_time+'" --to "' + end_time + '" --archive-path "'+path_to_archive+'" --video-codec MJPEG --audio-codec MP3 --span dvd --video-quality 100 --audio-quality 0 --fps-divider 100').connect(title='Утилита экспорта медиа данных')
