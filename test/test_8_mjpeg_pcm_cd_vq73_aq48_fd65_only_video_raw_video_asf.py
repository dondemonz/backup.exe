...
def test_30_H264():
    ...
    app = Application(backend="uia").start(r'"'+path_to_backupexe+'" --out "'+output_folder+out_file_name+'" --cam "' + cam_id + '" --from "'+begin_time+'" --to "' + end_time + '" --archive-path "'+path_to_archive+'" --video-codec MJPEG --audio-codec PCM --span cd --video-quality 73 --audio-quality 48 --fps-divider 65 --video-only --raw-video').connect(title='Утилита экспорта медиа данных')
