...
def test_30_H264():
    ...
    app = Application(backend="uia").start(r'"'+path_to_backupexe+'" --out "'+output_folder+out_file_name+'" --cam "' + cam_id + '" --from "'+begin_time+'" --to "' + end_time + '" --archive-path "'+path_to_archive+'" --video-codec MJPEG --audio-codec WMA --span 300 --video-quality 50 --audio-quality 50 --fps-divider 50 --silent').connect(title='Утилита экспорта медиа данных')
    ...
    #assert hasher.hexdigest() == hash_H264
...
def test_31_H263():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    #assert hasher.hexdigest() == hash_H263
    os.remove(''+output_folder + out_file_name+'')
...
def test_32_MPEG4():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    #assert hasher.hexdigest() == hash_MPEG4
    os.remove(''+output_folder + out_file_name+'')
...
def test_33_MJPEG():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    #assert hasher.hexdigest() == hash_MJPEG
    os.remove(''+output_folder + out_file_name+'')
...
def test_34_YUAN():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    #assert hasher.hexdigest() == hash_YUAN
    os.remove(''+output_folder + out_file_name+'')
...
def test_61_MxPEG():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    #assert hasher.hexdigest() == hash_MxPEG
    os.remove(''+output_folder + out_file_name+'')
