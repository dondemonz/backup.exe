...
def test_30_H264():
    ...
    app = Application(backend="uia").start(r'"'+path_to_backupexe+'" --out "'+output_folder+out_file_name+'" --cam "' + cam_id + '" --from "'+begin_time+'" --to "' + end_time + '" --archive-path "'+path_to_archive+'" --video-codec MPEG4 --audio-codec PCM --span cd --video-quality 60 --audio-quality 100 --fps-divider 70 --video-only').connect(title='Утилита экспорта медиа данных')
    ...
    assert hasher.hexdigest() == hash4_H264
...
def test_31_H263():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash4_H263
    os.remove(''+output_folder + out_file_name+'')
...
def test_32_MPEG4():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash4_MPEG4
    os.remove(''+output_folder + out_file_name+'')
...
def test_33_MJPEG():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash4_MJPEG
    os.remove(''+output_folder + out_file_name+'')
...
def test_34_YUAN():
    ...
    with open(''+output_folder + out_file_name+'', 'rb') as afile:
        ...
        hasher.update(buf)
    print(hasher.hexdigest())
    assert hasher.hexdigest() == hash4_YUAN
    os.remove(''+output_folder + out_file_name+'')
